...
show(io::IO, dt::DictTree)                                       = print(io, "DictTree with $(length(dt.layers)) active layer(s)")
show(io::IO, db::DictBranch)                                     = print(io, "DictBranch (prefix = $(repr(db.prefix))) $(length(collect(_sorted_branches(db)))) active layer(s)")
